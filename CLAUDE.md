...
- **Weak:** Qwen3-1.7B (HuggingFace: `Qwen/Qwen3-1.7B`)
- **Strong:** Qwen3-14B (HuggingFace: `Qwen/Qwen3-14B`)

Both are reasoning models that produce `<think>...</think>` blocks before answering.

## Dataset

AIME (1983-2024). For each model, we filter to problems in the "borderline" difficulty range — problems that model answers correctly 10–80% of the time (across multiple samples). This gives us problems where reasoning quality actually matters and there's variance to study.

## Experiment Design

1. **Baseline:** Each model solves problems end-to-end, multiple times, to establish per-problem accuracy rates
2. **Filtering:** Select problems in the 10–80% accuracy band for each model
...
- Local inference via HuggingFace `transformers` with `device_map="auto"` and `torch_dtype="auto"`.
- Need temperature > 0 for multiple samples per problem (currently 0.7, 16 samples).
- AIME answers are integers. We use `math-verify` for answer comparison with `\boxed{}` extraction.
- Truncation detection: if generated tokens == max_new_tokens, sample is discarded and retried.

## eval.py Usage

- `./eval.py --model {weak,strong}` — Baseline eval: find plausible (10–80% accuracy) problems for a model. Saves to `data/raw_results_{label}.jsonl` and `data/plausible_{label}.jsonl`.
...
Additional flags: `--max-tokens` (default 16384), `--batch-size` (sequences per generate call, default = n_samples).

## Data Files

- `data/raw_results_{label}.jsonl` — All evaluated problems for a model (full results).
- `data/plausible_{label}.jsonl` — Subset with accuracy in [0.10, 0.80].
...
**Phase 1: Baseline evaluation & problem filtering.** Run each model on AIME problems multiple times, compute per-problem accuracy, filter to the 10–80% band. Cross-evaluate models on each other's plausible sets. Select high-divergence target problems.
